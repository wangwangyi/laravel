    ...
    <div class="admin-content">
        <div class="admin-content-body">
            <div class="am-cf am-padding am-padding-bottom-0">
                <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">留言列表</strong> / <small>Comment</small></div>
            </div>
                        ...
                        <table class="am-table am-table-striped am-table-hover table-main">
                            ...
                            @foreach ($comments as $comment)
                                <tr>
                                    ...
                                    @if(!empty($comment->article->name))
                                    <td class="am-hide-sm-only">{{$comment->article->name}}</td>
                                    @endif
                            ...
                            @foreach ($del_comments as $com)
                                <tr>
                                    <td>{{$com->id}}</td>
                                    <td><a href="#">{{$com->username}}</a></td>
                                    <td class="am-hide-sm-only">{{$com->tel}}</td>
                                    <td class="am-hide-sm-only">{{substr($com->content,0,30)}}</td>
                                    @if(!empty($com->article->name))
                                        <td class="am-hide-sm-only">{{$com->article->name}}</td>
                                    @endif
                                    <td class="am-hide-sm-only">{{$com->created_at}}</td>
                                    <td>
                                        <div class="am-btn-toolbar">
                                            <div class="am-btn-group am-btn-group-xs">
                                                <a class="am-btn am-btn-default am-btn-xs am-text-secondary"
                                                   href="/admin/comment/{{ $com->id}}/del_show">
                                                    <span class="am-icon-pencil-square-o"></span> 查看
                                                </a>

                                                <a data-id="{{$com->id}}" class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only del">
                                                    <span class="am-icon-trash-o"></span> 删除
                                                </a>
                                            </div>
                                        </div>
